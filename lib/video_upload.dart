...
class _VideoUploadsScreenState extends State<VideoUploadsScreen> {
  ...
  String _sanitizeFileName(String fileName) {
    ...
    final extension = parts.length > 1 ? parts.last : 'mp4';
    ...
    String sanitized = nameWithoutExt
        ...
        .replaceAll(RegExp(r'[–—]'), '-') // Replace em-dash/en-dash
    ...
    final result = '${sanitized.isEmpty ? 'video' : sanitized}.$extension';
    print('Original: $fileName → Sanitized: $result');
    return result;
  }

  // ✅ Generate thumbnail from video (Web)
  Future<Uint8List?> _generateThumbnail(Uint8List videoBytes, String fileName) async {
    try {
      ...
      video.onLoadedMetadata.listen((_) async {
        try {
          ...
          final seekTime = video.duration != null && video.duration! > 0
              ? (video.duration! * 0.1).clamp(0, 2.0)
              : 1.0;
  ...
  Future<void> _uploadVideo() async {
    ...
    try {
      ...
      final sanitizedName = _sanitizeFileName(_selectedFile!.name);
      final fileName = '${DateTime.now().millisecondsSinceEpoch}_$sanitizedName';

      print('Uploading to: uploads/$userId/$fileName');
      ...
      _showSuccess('Video uploaded successfully with thumbnail!');
      _resetForm();
    } catch (e) {
